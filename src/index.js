...
function PhoneStorage() {
  ...
  React.useEffect(() => {
    axios
      .get("http://localhost:3000/unlisted/static/messageHistory.json")
      .then((res) => {
        if (!res.status === 200) {
  ...
  function addMessage(phone, msg) {
    ...
    setAllMessages((threads) =>
      threads.map((t) =>
        t.phone === phone ? { ...t, messages: thread.messages } : t
      )
    );
  }
...
ReactDOM.render(<PhoneStorage />, document.querySelector("#root"));
